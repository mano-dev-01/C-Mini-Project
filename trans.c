...
// NEW FEATURE: Calculate Monthly Interest Earnings
// Annual interest rate: 2% (0.02), Monthly: 0.02/12 = 0.00166667
#define ANNUAL_INTEREST_RATE 0.02
#define MONTHLY_RATE (ANNUAL_INTEREST_RATE / 12)
...
void calculateInterest(FILE *fPtr)
{
    ...
    printf("Enter account number to calculate interest (1 - 100): ");
    scanf("%d", &accountNum);

    if (!validateAccountBounds(accountNum))
        return;
    if (!authenticateAndLoadUser(fPtr, accountNum, &client))
        return;

    monthlyInterest = client.balance * MONTHLY_RATE;
    ...
    printf("Projected Balance (after 1 month): $%.2f\n", client.balance + monthlyInterest);
    printf("Projected Balance (after 1 year): $%.2f\n", client.balance * (1 + ANNUAL_INTEREST_RATE));
    printf("-------------------------------------\n");
}
...
void updateRecord(FILE *fPtr)
{
    ...
    if (!validateAccountBounds(account))
        return;
    if (!authenticateAndLoadUser(fPtr, account, &client))
        return;
...
void transferFunds(FILE *fPtr)
{
    ...
    scanf("%d", &acctFrom);
    if (!validateAccountBounds(acctFrom))
        return;
    if (!authenticateAndLoadUser(fPtr, acctFrom, &clientFrom))
        return;

    printf("Enter RECEIVER account number (1 - 100): ");
    scanf("%d", &acctTo);
    if (!validateAccountBounds(acctTo))
        return;
...
unsigned int enterChoice(void)
{
    ...
    printf("7 - Calculate monthly interest earnings\n");
